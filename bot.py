...
def start(update: Update, context: CallbackContext) -> None:
    update.message.reply_text(
        text='Welcome to URL downloader!\nPlease provide a valid url')
...
def textHandler(update: Update, context: CallbackContext) -> None:
    ...
    if update.message.parse_entities(types=MessageEntity.URL):
        ...
        update.message.reply_text(
            text=f'Your shotned url is: {shortenlink(user_message)}')
    else:
        update.message.reply_text(
            text='the url you sent is wrong, please retry !', quote=True)
...
def main():
    ...
    updater.dispatcher.add_handler(MessageHandler(
        Filters.all & ~Filters.command, textHandler, run_async=True))
    updater.start_webhook(listen="0.0.0.0", 
                        port=int(PORT),
                        url_path=TOKEN,
                        webhook_url="https://aymen-bot.herokuapp.com/" + TOKEN)
    updater.idle()
